 ...
 * @LastEditors: Lemon
 * @LastEditTime: 2021-05-06 21:18:30
...
const UserInfoModal:React.FC<UserInfoModalProps> = props => {
  ...
  return <>
    ...
    <Modal visible={visible} title={BtnStatusMap[type]} onOk={handleOk} onCancel={handleCancel}>
      <Form form={form}>
        <Form.Item name="id" style={{display: 'none'}}></Form.Item>
        <Form.Item name="email" rules={[{ required: true }]} label="邮箱">
          <Input placeholder='邮箱' />
        </Form.Item>
        <Form.Item style={{display: type !== 'update' ? '' : 'none'}} name="password" rules={[{ required: true }]} label="密码">
          <Input placeholder='密码' />
        </Form.Item>
        <Form.Item style={{display: type === 'login' ? 'none' : ''}} name="avatar" rules={[{ required: type !== 'login' }]} label="头像">
