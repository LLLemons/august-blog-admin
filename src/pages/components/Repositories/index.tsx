 ...
 * @LastEditors: Lemon
 * @LastEditTime: 2021-05-13 18:24:19
...
const Repositories: React.FC<RepositoriesProps> = (props) => {
  const { run, loading, pagination, data } = useRequest(
    (params) =>
      queryRepositories({
  ...
  const columns: ColumnsType<RepositoryNamespace.RepositoryModel> = [
    ...
    },
    {
      ...
      title: '创建时间',
      align: 'center',
      render: (v, r, i) => moment(v).format('YYYY-MM-DD HH:mm:ss'),
    },
    {
      dataIndex: 'updatedAt',
      title: '修改时间',
      ...
      render: (v, r, i) => {
        return (
          <>
            ...
            <a
              className="ml5"
              onClick={() => jumpToRepositoryPage({ id: r.id.toString() })}
            >
              详情
            </a>
          </>
